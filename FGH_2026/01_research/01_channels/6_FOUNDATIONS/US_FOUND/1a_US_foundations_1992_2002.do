...
** ***
// Step 3: Allocate to health focus areas
** ***
...
// Doing a key word search on the detailed grant description, activity description, and grant type description
	foreach var of varlist description type_tran activity_tran pop_grp_tran {
		...
		replace upper_`var' = subinstr(upper_`var', "�", "Y", .)
		replace upper_`var' = subinstr(upper_`var', "�", "Y", .)

		replace upper_`var' = subinstr(upper_`var', "�", "AE", .)
		replace upper_`var' = subinstr(upper_`var', "�", "AE", .)

		replace upper_`var' = subinstr(upper_`var', "�", "OE", .)
		replace upper_`var' = subinstr(upper_`var', "�", "OE", .) 

		replace upper_`var' = subinstr(upper_`var', "�", "C", .)
		replace upper_`var' = subinstr(upper_`var', "�", "C", .)

		replace upper_`var' = subinstr(upper_`var', "�", "N", .)
		replace upper_`var' = subinstr(upper_`var', "�", "N", .)

		replace upper_`var' = subinstr(upper_`var', "�", "SS", .)
	...
	***********
	// new keywords		
		run "FILEPATH"
		HFA_ado_master upper_description upper_activity_tran upper_type_tran upper_pop_grp_tran, language(english) keyword_path ("FILEPATH") channel(US_FOUNDATIONS)
