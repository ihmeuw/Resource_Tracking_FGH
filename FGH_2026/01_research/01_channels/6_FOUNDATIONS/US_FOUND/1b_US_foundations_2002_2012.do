...
// So here, I should add in the 2013 data to this .csv, right? (which is the standalone 2016 data)
	import delimited "FILEPATH",  clear varnames(1)  // Don't change this path - files not brought forward
	tempfile data
	save `data'
...
// clean data for merge
	...
	merge m:1 country_lc using "`CODES'/countrycodes_official.dta", keepusing(country_lc iso3)
	keep if _m==1 | _m==3
	drop _m
...
***********
	// new keywords		
		...
		HFA_ado_master description grant_subject_tran activity_override_tran grant_population_tran recip_subject_tran, language(english) keyword_path ("FILEPATH") channel(US_FOUNDATIONS)
