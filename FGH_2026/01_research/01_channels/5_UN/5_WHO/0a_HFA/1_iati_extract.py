...
REPORT_YEAR = 2024
# if True, rescrape all organizations even if they already exist in OUTPUT_DIR            
FORCE_RESCRAPE = True 
...
def save_meta(meta, fail_set, outdir):
    ...
    if fail_set:
        with open(outdir / "failed.txt", "w") as f:
...
if __name__ == "__main__":
    ...
    if not FORCE_RESCRAPE:
        print("  FORCE_RESCRAPE = False, Existing data will not be rescraped.")
    print("")
    #
    ...
    try:
        for (pub_code, name) in PUBLISHER_MAP.items():
            ...
            if (outdir / f"{pub_code}.csv").exists() and not FORCE_RESCRAPE:
                print(f"  {pub_code} has already been scraped. Skipping.")
                continue
            # Scrape the data
            xml_dir = Path(DATADUMP_DIR, f"iati-data-main/data/{pub_code}")
            ...
            if data is None:
                ...
                data.to_csv(outdir / f"{pub_code}.csv", index=False)
