#----# Docstring #----# ####
# Project:  FGH 
# Purpose:  Add donor information to COVID data
#---------------------# 
...
# data is less granular than our regular channel data.
...
cov <- collapse(cov, 'sum', c('recipient_country', 'iso3_rc', 'year'),
                c('total_amt', 'oid_covid_amt', 'swap_hss_other_amt', 'swap_hss_pp_amt'))
#----------------------------# 
...
regs[, oid_covid_amt := oid_covid_amt / n]
regs[, swap_hss_other_amt := swap_hss_other_amt / n]
regs[, swap_hss_pp_amt := swap_hss_pp_amt / n]
...
regs <- regs[, c('year', 'value', 'ihme_loc_id', 'total_amt', 'oid_covid_amt',
                 'swap_hss_other_amt', 'swap_hss_pp_amt')]
rm(isos)
source(paste0(code_repo, 'FUNCTIONS/helper_functions.R'))
...
regs <- collapse(regs, 'sum', c('recipient_country', 'iso3_rc', 'year'),
                c('total_amt', 'oid_covid_amt', 'swap_hss_other_amt', 'swap_hss_pp_amt'))
#---------------------------------# 
...
cov <- cov[iso3_rc != '',]
cov <- rbind(cov, regs)
cov <- collapse(cov, 'sum', c('iso3_rc', 'year'),
                c('total_amt', 'oid_covid_amt', 'swap_hss_other_amt', 'swap_hss_pp_amt'))
names(cov) <- toupper(names(cov))
cov[, INKIND:= 0]

adb <- fread(get_path('GFATM', 'fin', 'P_GFATM_ADB_PDB_FGH_precovid2024.csv'))
adb_merge <- adb %>%
  left_join(cov, join_by(YEAR, ISO3_RC, INKIND)) %>%
  group_by(ISO3_RC, YEAR, INKIND) %>%
  mutate(covidrows = sum(!is.na(TOTAL_AMT))) %>%
  mutate(TOTAL_AMT = TOTAL_AMT/covidrows,
         OID_COVID_AMT = OID_COVID_AMT/covidrows, 
         SWAP_HSS_PP_AMT = SWAP_HSS_PP_AMT/covidrows,
         SWAP_HSS_OTHER_AMT = SWAP_HSS_OTHER_AMT/covidrows) %>%
  mutate(TOTAL_AMT = ifelse(is.na(TOTAL_AMT), 0, TOTAL_AMT),
         OID_COVID_AMT = ifelse(is.na(OID_COVID_AMT), 0, OID_COVID_AMT), 
         SWAP_HSS_PP_AMT = ifelse(is.na(SWAP_HSS_PP_AMT), 0, SWAP_HSS_PP_AMT),
         SWAP_HSS_OTHER_AMT = ifelse(is.na(SWAP_HSS_OTHER_AMT), 0, SWAP_HSS_OTHER_AMT)) %>%
  mutate(DAH = DAH + TOTAL_AMT,
         oid_covid_DAH = OID_COVID_AMT,
         swap_hss_pp_DAH = swap_hss_pp_DAH + SWAP_HSS_PP_AMT,
         swap_hss_other_DAH = swap_hss_other_DAH + SWAP_HSS_OTHER_AMT) %>%
  select(-TOTAL_AMT, -OID_COVID_AMT, -SWAP_HSS_PP_AMT, -SWAP_HSS_OTHER_AMT, -covidrows)

save_dataset(adb_merge, paste0('P_GFATM_ADB_PDB_FGH_', dah.roots$report_year), 'GFATM', 'fin')
