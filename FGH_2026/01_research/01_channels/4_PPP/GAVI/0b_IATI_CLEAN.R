...
iati_simplify <- function(org_name,
  ...
  iati[sector == "", `:=`(
    scode = sector_code,
    sector_vocab = sector_vocab_cname,
    sector = sector_cname,
    sector_pct = sector_percentage
  )]
  setnames(iati, "scode", "sector_code")
  use_cols <- c(use_cols, "sector_code", "sector_vocab", "sector", "sector_pct")
  ...
  isos <- fread("FILEPATH/country_iso2to3.csv")
