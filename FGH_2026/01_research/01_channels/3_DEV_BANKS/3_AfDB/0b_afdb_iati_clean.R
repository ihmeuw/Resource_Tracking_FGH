#' Simplify IATI transaction data from iati-aggregator for use in AfDB estimates 
#'
...
#
._main <- function() {
    data_path <- get_path("afdb", "int", "afdb_iati_scrape.csv")
    output_path <- get_path("afdb", "int", "afdb_iati_clean.csv")
    
    log_threshold(logger::INFO)
    log_info("Simplify IATI transaction data")
    
    iati_simplify("afdb", data_path, output_path)
