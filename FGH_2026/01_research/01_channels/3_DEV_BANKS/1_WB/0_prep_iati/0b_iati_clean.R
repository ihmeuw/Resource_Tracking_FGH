#' Simplify IATI transaction data from iati-aggregator for use in WB estimates 
#'
...
iati_simplify <- function(org_name,
    ...
    ### knowing that the reported percentages should add up to 100. However, it
    ### is not uncommon for organizations to fail to follow this - in particular
    ### Hewlett appears to 
...
#
._main <- function() {
    for (pub_code in c("worldbank", "wbtf")) {
        data_path <- get_path("wb", "int", paste0(pub_code, "_iati_scrape.csv"))
        output_path <- get_path("wb", "int", paste0(pub_code, "_iati_clean.csv"))
        
        log_threshold(logger::INFO)
        log_info("Simplify IATI transaction data")
        
        iati_simplify(pub_code, data_path, output_path)
    }
