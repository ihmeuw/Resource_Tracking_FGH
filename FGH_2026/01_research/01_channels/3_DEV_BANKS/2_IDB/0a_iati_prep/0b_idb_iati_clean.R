#' Simplify IATI transaction data from iati-aggregator for use in IDB estimates 
#'
...
#
._main <- function() {
    data_path <- get_path("idb", "int", "iadb_iati_scrape.csv")
    output_path <- get_path("idb", "int", "iadb_iati_clean.csv")
    
    log_threshold(logger::INFO)
    log_info("Simplify IATI transaction data")
    
    iati_simplify("iadb", data_path, output_path)
