...
## Get draws
yvar_draws <- data.table(read_feather(metadata_list$retro_data_draws,
...
## Inverse logit and make means and UIs
chaos_list$draws[, paste0("draw_", c(1:metadata_list$N_draws)) := lapply(paste0("draw_", c(1:metadata_list$N_draws)), function(x) antilogit_fn(get(x), 0, 1))]
chaos_list$draws[chaos_list$draws < 0] <- 0.00001
