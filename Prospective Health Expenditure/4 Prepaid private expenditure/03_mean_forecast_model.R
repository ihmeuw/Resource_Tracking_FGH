...
error = function(e) {
  ...
  record_model_and_stop(model, variable = variable, version, date, type = "failed")
}
))
...
test_sig <- function (tmb_output_object, alpha = 0.1, intercept_test = T,
                      rigorous = F, avoid_coefs = NULL)
{
  crit_value <- abs(qnorm(p = alpha/2))
  pval_DT <- data.table(summary(tmb_output_object$FC_model,
                                p.value = T), keep.rownames = T)
  colnames(pval_DT) <- c("param", "est", "se", "zval", "pval")
  if (!rigorous) {
    pval_DT <- pval_DT[param %in% c("b", "a", "logSigma",
                                    "logggroup", "loggrcoef", "c", "scaled_c", "rho_global",
                                    "logSigma_rho_country", "theta_global", "var_theta_global",
                                    "var_theta_country")]
  }
  else {
...
stat_sig_test <- test_sig(output_TMB, rigorous = T, avoid_coefs = c("a", "loggroup"))


## Test for growth rate bounds
## Run SFA models first
sfa_model <- sfa_bounds(
  data = input_data, depvar = model_info$yvar,
...
fwrite(postfile_iso, paste0(root_fold, "FILEPATH"))

## Save the model text file in passed
record_model_and_stop(model, variable = variable, version, date, type = "passed")
