...
########################################
#### (13) Make OOP per capita = OOP/GDP * GDPpc
########################################

oop_pc_draws <- merge(correlated_output, gdppc[, .(iso3, year, gdppc)], c("iso3", "year"), all.x = T)
oop_pc_draws[, paste0("draw_", c(1:metadata_list$N_draws)) := lapply(paste0("draw_", c(1:metadata_list$N_draws)), function(x) get(x) * gdppc)]
oop_pc_draws[, gdppc := NULL]

## Calculate means and UIs
oop_pc_stats <- stat_maker(oop_pc_draws, melt = T, merge = F, idvar = c("iso3", "year"))


#### Make scenarios ####

## Get mean scenario lines
scenario_mean_lines <- mean_scenario_model(oop_pc_stats, metadata_list,
