###################################################
### Author: USERNAME
### Purpose: Preparing all the inputs going into OOP forecasting
###################################################
...
###########################
### (4) Get DAH per GDP, GHE per GDP, and PPP per GDP
###########################
...
ghe <- fread('FILEPATH')
ghe <- ghe[, .(iso3, year, ghe_gdp = mean)]

ppp <- fread('FILEPATH')
ppp <- ppp[, .(iso3, year, ppp_gdp = mean)]
...
# Subset here through the last observed year
oop <- fread(retro_data_stats)[year >= start_year & year <= 2022, .(iso3, year, oop_gdp = mean)]
oop <- oop[!grepl("_", iso3)]
...
data <- merge(data, oop, c("iso3", "year"), all.x = T)
...
data[, logit_ghe_gdp := logit(ghe_gdp)]
...
data[, logit_oop_gdp := logit(oop_gdp)]
