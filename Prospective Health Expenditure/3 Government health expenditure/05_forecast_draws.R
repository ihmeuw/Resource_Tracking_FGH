...
## Get mean grid
root_fold <- paste0('FILEPATH')

## Get mean grid
load(paste0(root_fold, "FILEPATH"))

if(!(id %in% rmse_distro$array_grid$id)) {
  
  print(paste0("Model number ", id, " did not pass inclusion criteria; no draws will be run"))
...
diag(use_prec_mat) = diag(use_prec_mat)*10
