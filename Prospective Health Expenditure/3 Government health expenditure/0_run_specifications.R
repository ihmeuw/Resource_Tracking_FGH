...
# Input filepath to retrospective pipeline GHE total outputs
retro_data_stats <- 'FILEPATH'
retro_data_draws <- 'FILEPATH'
new_retro_data_draws <- 'FILEPATH'
input_data_path <- 'FILEPATH'

# Filepath to forecasting input data
prepped_inputs <- 'FILEPATH'
...
ensemble_metadata <- list(
  yvar = c(yvv),
  xvar = paste0("FE_", c("ln_gdppc", "logit_pop64", "logit_gge_gdp", "logit_dah_gdp")),
  re_coef = NULL,
  ar = c(0, 1, 2),
  ar_mod = c(1),
  ma = c(0),
