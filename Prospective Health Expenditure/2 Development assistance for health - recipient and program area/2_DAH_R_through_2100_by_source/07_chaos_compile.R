...
source("FILEPATH/country_looper_hack.R")
...
system.time(draws_data <- foreach(f = rmse_distro$array_grid[, model_number]) %dopar% {
  ...
  ## Load the diff draws
  return(data.table(read_feather(paste0(root_fold, "FILEPATH"))))
})
...
################################################
## (4) Compute and cumulative sum
################################################


if (metadata_list$chaos == "T" | metadata_list$chaos == T | metadata_list$chaos == "TRUE") {
  print("Compile the diff draws for a single OOS unit and Chaos them")
  system.time(all_draws_for_now <- country_looper_hack(
    oos_input = oos_year, years = year_oos[[oos_year]], draw_type = "diff",
    input_rmse_data = rmse_distro$data, draws_data = draws_data
  ))
  
  if (oos_year == 1) {
    print("Get just the first year of draw in levels if OOS == 1")
    system.time(first_lev_year <- country_looper_hack(oos_year, metadata_list$start_year,
                                                      draw_type = "level",
                                                      input_rmse_data = rmse_distro$data, draws_data = draws_data
    ))
